...
To parse dates use parse(), e.g.::
    ...
    >>> parse('1890')
    FlexiDate(year=u'1890')
    >>> parse('1890?')
    FlexiDate(year=u'1890', qualifier='Uncertainty: 1985?')
...
class FlexiDate(object):
    ...
    def isoformat(self, strict=False):
        """
        Convert FlexiDate to isoformat (same as __str__ but without qualifier)
        :returns date in isoformat (same as __str__ but without qualifier).
    ...
    #  not necessary to keep regex sting in var?  our_re_pat =
    our_re = re.compile('''
        (?P<year> -?[\d?]+)
        (?:
                \s* - (?P<month> [\d?]{1,2})
...
def parse(date, dayfirst=True):
    """
    Parse a unicode imprecise date into a FlexiDate.

    :param date: the date to parse - may be a unicode string, datetime.date,
    datetime.datetime or FlexiDate.

    .. todo:: support for quarters e.g. Q4 1980 or 1954 Q3
    .. todo:: support latin stuff like M.DCC.LIII
    .. todo:: convert '-' to '?' when used that way e.g. had this date [181-]
    """
